//! CP ↔ Gateway mTLS channel construction (TLS 1.3, mutually authenticated; VERSIONING §7, Design §10/§15).
//! Bootstrap channel (no client cert): EnrollGateway + Handshake.Negotiate (bootstrap exception).
//! mTLS channel (client cert): RenewGatewayIdentity + SessionSigning.SignSessionCertificate.
//! Fail-closed (NFR-2): missing/invalid/expired/wrong-CA/plaintext/TLS-1.2 rejected; time-bounded every phase (§10.3).
//! TLS-1.3 enforced via custom ServerCertVerifier rejecting TLS-1.2 handshake-signature path; client key never leaves Gateway (D2/§15).
