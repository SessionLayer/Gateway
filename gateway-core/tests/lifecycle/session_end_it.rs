...
async fn session_end_signal_on_every_teardown_path_and_exact_leases() -> anyhow::Result<()> {
    ...
    let (gw, sd, _live_sessions) =
        start_gateway(&cp, Arc::new(gw_config(MidSessionExpiryMode::RunToTtl))).await;
    ...
    let (gw, sd, _live_sessions) =
        start_gateway(&cp, Arc::new(gw_config(MidSessionExpiryMode::HardKill))).await;
    ...
    let (gw, sd, live_sessions) =
        start_gateway(&cp, Arc::new(gw_config(MidSessionExpiryMode::RunToTtl))).await;
    cp.set_grant_expiry(now_epoch() + 3600);
    // Real state, not a fixed delay: the channel-admission registry (`ensure_registered`)
    // is populated when the Gateway commits to the SESSION, which precedes the shell
    // channel opening -- firing on its first instant lands an open-refusal instead of the
    // live teardown under test. Require it to hold across polls, and let the shell emit its
    // marker repeatedly, so this does not depend on hitting one exact moment.
    ...
    cp.push_lock_when(
        ...
        move || {
            live_polls = if live_sessions.is_empty() {
                0
            } else {
                live_polls + 1
            };
            live_polls >= 120 // ~3s continuously live, at the helper's 25ms poll
        },
    );
    let (code, stdout, _stderr) = ssh_run(
        &client,
