...
/// A registered break-glass key and an unregistered one must cost the SAME number of CP
/// round-trips. Resolving the pin only after a break-glass miss made the registered case
/// answer in one RPC and the unregistered case in two, so anyone holding a security key's
/// private material could learn by latency alone whether it is provisioned for emergency
/// access -- without tripping the alert path that makes break-glass probing visible.
///
/// Asserts the RPC COUNT, not the auth outcome: the outcomes already differ (that is the
/// point of authentication), so a test that only checked accept-vs-reject would pass
/// against the leaking implementation.
#[tokio::test]
async fn break_glass_probe_costs_the_same_rpcs_whether_or_not_the_key_is_registered() {
    async fn rpcs_for(registered: bool) -> (usize, usize) {
        let cp = MockCp::start().await;
        let config = Arc::new(SshServerConfig {
            listen_addr: "127.0.0.1:0".to_string(),
            login_grace_secs: 30,
        ...
        let conn = Arc::new(ConnState::default());
        let mut handler = SshHandler::new(deps, "10.9.9.9".parse().unwrap(), conn);
        let _ = handler.auth_publickey("deploy%node-bg", &sk).await;
        cp.auth_rpc_counts()
    }

    let hit = rpcs_for(true).await;
    let miss = rpcs_for(false).await;
    assert_eq!(
        hit, miss,
        "a registered break-glass key cost {hit:?} (resolve_pin, resolve_breakglass_key) \
         RPCs and an unregistered one cost {miss:?}; the difference is a timing oracle \
         disclosing which security keys are provisioned for break-glass"
    );
}
...
async fn start_gateway(
    ...
    let deps =
        crate::support::outer_leg_deps_with(cp, config.clone(), connector, RecorderChoice::Real)
            .await;
