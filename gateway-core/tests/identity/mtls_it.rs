use crate::support::{MockCp, TestCa};
...
async fn plaintext_peer_is_refused() {
    let (endpoint, _guard) = crate::support::spawn_plaintext_server().await;
...
async fn expired_server_cert_is_refused() {
    ...
    let cfg = crate::support::raw_server_config(leaf.cert_der, leaf.key_pkcs8_der, None);
    let (endpoint, _guard) = crate::support::spawn_raw_tls_server(cfg).await;
...
async fn tls12_only_server_is_refused() {
    ...
    let cfg = crate::support::raw_server_config(
        leaf.cert_der,
        leaf.key_pkcs8_der,
...
async fn hung_peer_is_bounded_by_timeout() {
    let (endpoint, _guard) = crate::support::spawn_silent_server().await;
    let anchor = TestCa::generate("cp").cert_der().to_vec();
