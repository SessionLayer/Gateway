...
/// [SessionLayer patch] The `*-cert-v01@openssh.com` host-key ALGORITHMS for each
/// configured host certificate whose certified public key matches one of the
/// server's host keys, as `Algorithm::Other(<cert-type>)` (which round-trips its
/// full cert-type string through `to_string()`, unlike a base `Algorithm` which
/// drops the cert marker). Both the advertise (`write_kex`) AND the select
/// (`read_kex`) must include these, else the server offers the cert algorithm but
/// selects the plain one, and a client that negotiated the cert alg gets a key-type
/// mismatch. Empty (the default) ⇒ plain host-key behaviour, unchanged.
#[cfg(not(target_arch = "wasm32"))]
pub(crate) fn cert_host_key_algorithms(keys: &[PrivateKey], certs: &[Certificate]) -> Vec<Algorithm> {
    certs
        .iter()
        .filter(|cert| keys.iter().any(|k| k.public_key().key_data() == cert.public_key()))
        .filter_map(|cert| cert.algorithm().to_certificate_type().parse::<Algorithm>().ok())
        .collect()
}

#[cfg(target_arch = "wasm32")]
pub(crate) fn cert_host_key_algorithms(_keys: &[PrivateKey], _certs: &[Certificate]) -> Vec<Algorithm> {
    Vec::new()
}
...
pub(crate) trait Select {
    ...
    /// [SessionLayer patch] `host_certificates` adds `*-cert-v01@openssh.com` algorithms
    /// for configured host certs so the server can SELECT (not just advertise) them.
    fn read_kex(
        buffer: &[u8],
        pref: &Preferred,
        available_host_keys: Option<&[PrivateKey]>,
        host_certificates: &[Certificate],
        cause: &KexCause,
    ) -> Result<Names, Error> {
        ...
        let possible_host_key_algos = match available_host_keys {
            // [SessionLayer patch] Offer cert algorithms FIRST for configured host
            // certs, then the plain-key algorithms — so the server selects the cert
            // alg it advertised (matching a client that negotiated it; no key-type
            // mismatch). `cert_host_key_algorithms` is empty in the default case.
            Some(available_host_keys) => {
                let mut algos = cert_host_key_algorithms(available_host_keys, host_certificates);
                algos.extend(pref.possible_host_key_algos_for_keys(available_host_keys));
                algos
            }
...
pub(crate) fn write_kex(
    ...
    writer.packet_bytes(|w| {
        ...
        if let Some(server_config) = server_config {
            // [SessionLayer patch] Advertise a `*-cert-v01@openssh.com` algorithm for
            // each configured host certificate whose key we hold (offered FIRST so a
            // client with `@cert-authority` negotiates it — the host-cert MITM path,
            // no TOFU), then the plain host-key algorithms from `prefs.key` we have
            // keys for. The cert algorithms are not in russh's default `prefs.key`, so
            // they must be added here rather than merely filtered in.
            let mut names: Vec<String> = Vec::new();
            for cert_algo in
                cert_host_key_algorithms(&server_config.keys, &server_config.host_certificates)
            {
                let name = cert_algo.to_string();
