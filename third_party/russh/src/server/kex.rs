    ...
    pub async fn step<H: Handler + Send>(
        ...
        match self.state {
            ServerKexState::Created => {
                ...
                let names = {
                    ...
                    negotiation::Server::read_kex(
                        ...
                        // [SessionLayer patch] configured host certs → cert host-key algos.
                        &self.config.host_certificates,
                        &self.cause,
                    )?
                };
            ...
            ServerKexState::WaitingForDhInit { mut names, mut kex } => {
                ...
                // [SessionLayer patch] If the negotiated host-key algorithm is a
                // `*-cert-v01@openssh.com` type and a matching host certificate is
                // configured, present the CERTIFICATE as K_S (host-cert MITM path); the
                // plain host key still signs the exchange hash. `host_cert` is the cert
                // whose certified key we sign with, or None for the plain-key path.
                let host_cert = self.config.host_certificates.iter().find(|cert| {
                    cert.algorithm().to_certificate_type() == names.key.to_string()
                        && self
                            .config
                            .keys
                            .iter()
                ...
                let Some(matching_key_index) = self.config.keys.iter().position(|key| {
                    is_key_compatible_with_algo(key, &names.key)
                        || host_cert.is_some_and(|c| c.public_key() == key.public_key().key_data())
                }) else {
                ...
                let hash = HASH_BUF.with(|buffer| {
                    ...
                    // [SessionLayer patch] K_S is the certificate blob when presenting a
                    // host cert, else the plain host public key. It MUST be byte-identical
                    // to what is written on the wire below (the client hashes what it sees).
                    match host_cert {
                        Some(cert) => cert.to_bytes()?.encode(&mut pubkey_vec)?,
                        None => key.public_key().to_bytes()?.encode(&mut pubkey_vec)?,
                    }
                ...
                output.write_packet(|w| {
                    ...
                    .encode(w)?;
                    // [SessionLayer patch] K_S on the wire — the cert blob or the plain
                    // host key, matching the exchange-hash input above exactly.
                    match host_cert {
                        Some(cert) => cert.to_bytes()?.encode(w)?,
                        None => key.public_key().to_bytes()?.encode(w)?,
                    }
