...
pub struct Config {
    ...
    /// [SessionLayer patch] Host CERTIFICATES to present instead of a plain host
    /// key. When the client negotiates a `*-cert-v01@openssh.com` host-key
    /// algorithm, the server sends the matching certificate (matched to a key in
    /// `keys` by public key) as K_S and still signs the exchange hash with the
    /// plain private key. Empty (default) ⇒ plain host-key behaviour, byte-for-byte
    /// unchanged. See `src/server/kex.rs` and `src/negotiation.rs`.
    pub host_certificates: Vec<Certificate>,
