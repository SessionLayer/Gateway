...
accept, so the run fails at the first boot-guard case rather than further in. That is the check
...
The wrong-key scenario uses LocalStack's `_custom_id_` tag to rebuild the pinned ARN on top
of a second, unrelated keypair. Against real KMS that is the shape a repointed **alias**
produces, which is why the key-reference grammar refuses aliases outright; it cannot be
produced by rotating a real KMS key, because asymmetric key material never rotates in
place. The scenario asserts the impostor genuinely differs from the pinned key before
trusting anything it concludes — if LocalStack ever returned the same ARN with the same
material, every later assertion would pass while testing nothing.
